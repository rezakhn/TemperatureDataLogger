...
void create_dashboard() {
  ...
  for(int i = 0; i < tempData.sensorCount; i++) {
    lv_chart_series_t *ser = lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_RED + i), LV_CHART_AXIS_PRIMARY_Y);
    lv_chart_set_series_color(chart, ser, lv_palette_main(LV_PALETTE_RED + i));
  }
...
void create_settings_screen() {
  ...
  lv_obj_t *tabview = lv_tabview_create(settings_screen, LV_DIR_TOP, 40);
...
void update_chart() {
  ...
  // Clear the chart
  lv_chart_set_all_value(chart, LV_CHART_POINT_NONE);
    ...
    if(tempData.dataCount[i] > 0) {
      ...
      if(series) {
        for(int j = startIndex; j < tempData.dataCount[i]; j++) {
          ...
          if(temp != DEVICE_DISCONNECTED_C) {
            lv_chart_set_value_by_id2(chart, series, j - startIndex, temp * 100, 0);  // Scale temperature for display
          }
        }
      }
    }
